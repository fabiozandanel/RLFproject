...
\abstract{
The underlying physics of giant radio halos and mini halos is still an open
question despite the growing number of detections. In this paper, we explore the
possibility that the radio-emitting electrons are generated in hadronic cosmic
ray (CR) proton interactions with ambient thermal protons of the intra-cluster
medium. Our CR model derives from cosmological cluster simulations and
additionally accounts for CR transport in the form of CR streaming and diffusion
that opens the possibility of changing the radio halo luminosity by an order of
magnitude on a dynamical time scale. We draw our cluster sample from the large
cosmological ``MultiDark'' simulation and construct a phenomenological gas
density model based on X-ray measurements that matches Sunyaev-Zel'dovich and
X-ray scaling relations and luminosity functions. Our model successfully
reproduces the observed surface brightness profiles of giant radio halos (Coma,
A2163) as well as radio mini-halos (Perseus, Ophiuchus) while obeying upper
limits on the gamma-ray emission in these clusters. Because of CR streaming
transport and the different scalings of the X-ray luminosity $L_{\rmn{X}}$ and
the Sunyaev-Zel'dovich flux, $Y$, with gas density, our model is also able to
simultaneously reproduce the observed bimodality of radio-loud and radio-quite
...
apparent contradiction. For a plausible fraction of 10\% radio-loud clusters,
our model matches the NVSS radio halo luminosity function. Constructing an
analytical radio-halo luminosity function, we demonstrate the unique prospects
for low-frequency radio surveys (such as the LOFAR Tier 1 survey) to detect
$\sim3500$ radio halos back to redshift two and to probe the underlying physics
of radio halos.
}
...
\begingroup
\let\thefootnote\relax\footnotetext{ * fabio@iaa.es, christoph.pfrommer@h-its.org}
\endgroup
Diffuse cluster radio emission can be separated in two classes: radio halos and
radio relics (see, e.g.~\citealp{2004rcfg.procE..25K,2008SSRv..134...93F}).
...
\citealp{2009A&A...507..661B,2011A&A...527A..99E}.  This suggests the existence
...
current observational constraints and predicts luminosity functions for the LOFAR
cluster survey.
...
observed X-ray and SZ scaling relations and luminosity function. We then
...
those to the corresponding BCS XLFs and to our model predictions. Note that
there is only the Schechter fit available for the BCS bolometric XLF.  While the
...
luminosities. This is also true in the bolometric band, where the XLFs of
\cite{2010MNRAS.406.1773M} and our model agree well, but deviate from the BCS
Schechter fit at low luminosities. This may be an artifact due to the use of
Schechter fit instead of the data points or may point to incompleteness of the
BCS sample. Note that the Poissonian errors of the XLF obtained from the
MultiDark simulation are a lower limit as we are neglecting the uncertainty due
to cosmic variance.  Studies of the XLF will become again an important topic
with the upcoming launch of the eROSITA satellite (see
...
\label{sec:2.3}
We assume a power-law CR proton distribution, $f(R,p) \dd p=C(R) p^{-\alpha} \dd p$,
...
distance to the RH. Note that we do not convolve $S_\nu$ with the instrumental
...
the spatial CR distribution has to be modified to include the effects of CR
streaming and for that reason, we take the analytical result from
...
\caption{Left panel: we show our final hybrid model profile for the
  ...
  conservation during CR streaming. Right panel: we compare the
...
it accounts for (i) X-ray-inferred (CCC and NCCC) gas profiles and cluster-mass
...
In the left two panels of Figure~\ref{fig:CRFinalModel}, we show our final
hybrid CR normalization for the NCCC and CCC cases and for different values of
...
irrespective of the the cluster state. While turbulence in NCCCs could be
...
In the right panel of Figure~\ref{fig:CRFinalModel}, we compare our final hybrid
...
\citealp{2010MNRAS.409..449P}) and with the exact analytical solution as in
\citet{2011A&A...527A..99E}, but for our GNFW profiles (adopting $\alpha=2.3$
and $\gamma_{\rmn{tu}}=100$, see Appendix~\ref{app:B} for details).  The
profiles are normalized at $0.1 R_{200}$. In that case of dominant advective CR
transport, our final model compares nicely with the semi-analytical model
derived from cosmological cluster simulations \citep{2010MNRAS.409..449P}.  The
main differences between our final model (and the semi-analytical model) on the
...
clusters. Note that our final model profiles are generally more centrally peaked
...
assumptions about the CR profile in the advection-dominated regime and other
...
\footnotesize{Note. Top two rows correspond to giant radio halos, while the
  ...
  not critical for the surface brightness modeling. The fit parameters column
  indicates the best fit values (top row of each cluster) and other permitted
...
The best fit to the data is obtained for $\gamma_{\rmn{tu}}=3$ and
...
\item The radio mini-halos in Perseus and Ophiuchus can be easily reproduced in
  ...
  are three possibilities that explain this apparent contradiction.  (i) Primary
  electrons accelerated in outer shocks, which we do not consider here, could
  have an important contribution to the RH emission, particularly in the cluster
  outskirts \citep{2008MNRAS.385.1211P}. While this does not have to
  ...
  seen in Coma and Abell~2164, requiring inherently non-spherical modelling. In
  ...
  carefully quantified in future work.  (iii) Adopting the simulation-derived
  ...
  the overcooling problem of purely radiative simulations. This produces cluster
  cores that are too dense (in comparison to observations), which also should
  overestimate the rate of adiabatic compression that is experienced by the CR
  population during the formation of the cooling core. Hence, the resulting 
  values of $\gamma_{\rmn{tu}}$ are then biased low in comparison to a
  potentially shallower slope of the inner CR profile. To quantify the last
  point, we fit the Coma surface brightness using a model without
  $\tilde{C}$. We find that values as high as $\gamma_{\rmn{tu}} \approx 8$ can
  be accommodated. However, $\gamma_{\rmn{tu}}=1$ still represents the best fit
  model, demonstrating that the problem can be weakened but not circumvented
  even in this case of a cored CR profile. Nevertheless, we can successfully
  reproduce the morphology of Coma and Abell~2163 RHs fairly well, solving the
  previous problems of the \emph{classical} hadronic model (see
  e.g.~\citealp{2010MNRAS.401...47D}).
...
\caption{Radio-to-X-ray and radio-to-SZ scaling relations as predicted by our
  ...
  for all cases. See main text for details. \C{change colors?}}
\label{fig:SR}
\end{figure*}
...
(e.g.~A2029 and A1835), has a radio luminosity which is much lower than giant
...
other hand, X-ray luminosities for the same object as derived by, e.g.,~ROSAT
and \emph{Chandra} can easily differ by a factor of a few \C{CITE}. In the left
panel of Figure~\ref{fig:PLSZ}, we additionally show the model of
...
would produce slightly too concentrated surface brightness profiles of giant
radio halos. As laid out in detail in Sect.~\ref{sec:discussion_hadronic}, this
may be an artifact caused by a combination of three factors, namely additional
primary radio emission in the cluster periphery, an enhanced non-thermal
clumping factor, and our choice of simulation-derived CR profiles that may be
too concentrated as a result of the overcooling in radiative simulations without
AGN feedback. Moreover, we demonstrate in Figure~\ref{fig:SR} that the
normalization of the RH luminosity is highly degenerate with our other
parameters so that a different combination of those could mimic a similar
normalization for smaller values of $\gamma_{\rmn{tu}}$.
...
\caption{Radio luminosity function $n(L,z)$ at 120~MHz (top left panel) and cumulative
...
for this wave length regime in our model, which we will do next.
...
line). We limit the integral to luminosities $\log_{10} L_{120~\rmn{120MHz}}
...
The reason for this lies in the steep mass function of halos which selects the
halos above any cutoff (that is either physically or observationally realized
through a survey flux limit) to dominate the total number of (detectable)
RHs. Only if the radio luminosity scaling remains unaltered below the survey
flux limit, the steepness of the mass function ensures that there will be more
RHs scattered above the flux limit then below it, the so-called Eddington bias
that causes the inferred luminosities based on only the detected sources to end
up as an overestimate. Hence the presence of any hypothetical break in the radio
luminosity scaling, which is unconstrained by current data, explains the largely
varying model predictions in the recent literature, which vary from a few to
hundreds observable RHs \citep{2010A&A...509A..68C,2011arXiv1110.2786S} to
thousands of detectable RHs in future surveys \citep{2002A&A...396...83E}.
...
\section{Conclusions}
\label{sec:6}
We present a detailed model for the RH population assuming that the
radio-emitting electrons are generated through hadronic CR interactions with
thermal protons of the ICM. We improve over previous works by
...
luminosity functions. We confirm theoretical ideas suggested by
...
thermal X-ray emission and the bimodality of radio-loud and radio-quite clusters
at a fixed X-ray luminosity.

To this end, we use a complete cosmological sample of galaxy clusters from the
MultiDark, DM-only $N$-body simulation \citep[MultiDark][]{2011arXiv1104.5130P}
and employ seven snapshots from $z = 0$ up to $z = 1$. We select all DM halos
with a mass above $M_{200}\approx1.4\times10^{14}$~$h_{70}^{-1}$~M$_{\odot}$. In
a next step, we construct a \emph{phenomenological} model for the cluster-gas
distribution. This is characterized by X-ray-inferred (CCC and NCCC) gas
profiles \citep[of the REXCESS sample][]{2008A&A...487..431C}, a
cluster-mass-dependent gas fraction \citep{2009ApJ...693.1142S}, and we adopt
the universal temperature drop in the outskirts of clusters seen in simulations
and deep X-ray observations. We assign such a (cluster mass-dependent) gas
density profile to a DM halo and sort it into the NCCC/CCC populations according
to a dynamical disturbance parameter that is calculated for the DM distribution.
In this way, we obtain a cosmologically complete mock catalog of galaxy clusters
that matches the observed $L_{\rmn{X, bol}}$-to-mass, $Y_{\rmn{X}}$-to mass and
$Y_{\rmn{SZ}}$-to-mass relations, and the X-ray luminosity function.
...
transport. This model is supplemented with an analytical model
\citep{2011A&A...527A..99E} that includes the treatment of microscopic CR
transport processes. While turbulently-driven CR advection leads to centrally
enhanced CR profiles, CR propagation in form of CR streaming and diffusion
produces flatter CR profiles, which dominates for decaying cluster
turbulence. In our model, we introduce a CR propagation parameter
$\gamma_{\rmn{tu}}$ which is the ratio of the CR streaming-to-advection time
scale and allows us to effectively switch the regimes where either process
dominates the CR transport and to explore different turbulent states of the
clusters.
...
mini radio-halos, solving problems of the \emph{classical} hadronic model. We
recover the total radio luminosity within 20\% for magnetic field configurations
favored by Faraday rotation measurements. We demonstrate that within the
parameter ranges that provide acceptable fits to the radio profiles, the
resulting gamma-ray emission from the decay of neutral pions---an inevitable
byproduct in hadronic CR interactions---is well below observational gamma-ray
upper limits from {\em Fermi}, and imaging air Cherenkov telescopes, MAGIC,
VERITAS, and H.E.S.S. Future more sensitive gamma-ray observations may in
principle be able to disentangle the two competing models for RHs, the hadronic
and re-acceleration scenarios.
...
fits prefer lower values of $1\lesssim\gamma_{\rmn{tu}}\lesssim5$. However, the
...
the X-ray emission maps), (iii) our choice of simulation-derived CR profiles
that may be too concentrated as a result of the overcooling in radiative
simulations without AGN feedback, and (iv) a possible spectral steepening in the
cluster core region due to CR streaming and diffusive transport that could cause
spatial variations of the CR spectral index throughout the cluster and hence a
spatially varying radio emission, in particular in the cluster outskirts. 
...
reproduce the observed bimodality of radio-loud and radio-quite clusters at the
...
model RLF subject is subject to cosmic variance because of the comparably small
simulation volume of $1 h^{-3}\,\rmn{Gpc}^3$. Interestingly, the RLFs derived
from the $L_{1.4~\rmn{GHz}}-L_{\rmn{X,bol}}$ and $L_{1.4~\rmn{GHz}}-Y_{\rmn{SZ},
  500}$ relations differ at high radio luminosities from the NVSS RLF; possibly
because of selection and incompleteness effects that are not fully taken into
account. The comparison between different RLFs illustrates that the
low-luminosity (low-mass) regime is the most promising place to differentiate
between different models.

It is expected that the next-generation of low-frequency surveys probe this
regime; hence, we present the 120~MHz RLF and cumulative number density, and
make predictions for the LOFAR cluster survey. Given our assumption, we would
expect the LOFAR Tier~1 survey at 120~MHz to detect about $\sim$3500 RHs above
the flux limit of 0.5~mJy. The precise number depends strongly on the underling
assumptions and, in particular, that the model parameters can be extrapolated
down to cluster masses of about
...
unconstrained by current observations. If, e.g., the magnetic field distribution
in clusters is not statistically self-similar and would exhibit much reduced
strength on group scales, our predictions for the detectable RHs would be
dramatically reduced. 

This demonstrates the potential of LOFAR and other next-generation
low-sensitivity radio instruments, in determining the RLF properties over a
broad range of luminosities. In combination with future X-ray missions like
eROSITA, this should yield a robust determination of the number of clusters
hosting RHs at a given luminosity (mass) and thus elucidate the relation of the
radio emission with cluster dynamical state. This will be crucial in
determining the parameter values for the hadronic model and eventually in
elucidating the RH generation mechanism.

Concluding, in this work we constructed a \emph{phenomenological} model for the
ICM and CR distribution that enabled us to provide cosmologically complete
multi-frequency mock catalog for the (non-)thermal cluster emission at different
redshifts. We will make these catalogs publicly and freely available
on-line. Those contain the quantities $\rho_{\rmn{gas}}$, $L_{\rmn{X, bol}}$,
$Y_{\rmn{X}}$, $Y_{\rmn{SZ}}$, $L_{120~\rmn{GHz}}$, $L_{1.4~\rmn{GHz}}$ and
$L_{\gamma}$ (among many others). We hope that the community can make valuable
