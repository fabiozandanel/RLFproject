...
\begin{abstract}
  ...
  require a flat CR profile (A2163); {\bf provided that the zero point of the radio
  ...
  halos are primarily of hadronic origin, (ii) {\bf giant halos transition from
  ...
  spectrum radio sources are mainly of leptonic origin. {\bf To test this model}, we
...
reduces the CR bulk speed. {\bf Balancing the growth rate of the CR Alfv{\'e}n wave
...
{\bf We use X-ray-inferred gas densities $\rho_{\rmn{gas}}$ and temperatures 
...
\caption{Surface brightness profiles of the Coma radio halo, normalized at 300
  kpc. We compare the 352 MHz profile of \citet{2012arXiv1207.3025B}, that suffer from
  residual point source contributions (black squares), to profiles where an
  adaptive filtering technique was applied to remove the residual point sources
  (filled blue squares, Rudnick, priv. comm.). Increasing the zero level of the
  ...
  band for \citealp{2012arXiv1207.3025B}).}
\label{fig:SB_Coma}
\end{figure}
...
\item Merging clusters are not spherically symmetric as can be seen in Coma and
  ...
  anisotropies, it may be inadequate to model small-scale inhomogeneities {\bf such
...
modification of this purely hadronic model in explaining RHs, {\bf which we will then
...
by hadronic interactions and the other is of leptonic origin. {\bf While we leave the
...
\item The case of both components significantly contributing to the diffuse
  ...
  hadronic and leptonic component. {\bf This is generalizing the simulation-inspired
...
\item {\em Spectral and morphological variability.} In mini-halos and in the
  ...
  {\bf CR advection effects} into account, which would mix regions of different CR
  ...
  discrete weak shocks or turbulent acceleration), {\bf the expected distribution of
  Mach numbers respectively CR momentum diffusion coefficient}, and the
  comparably short electron cooling time ($\sim100$~Myr). {\bf Interestingly, this
  ...
  towards the West) in their high-resolution surface brightness map. This may
...
\item {\em Halo switch-on/-off mechanism and the radio-X-ray bimodality.}
  ...
  hadronic and leptonic halo components---{\bf which we will scrutinize now}.
\end{enumerate}
...
the Coma halo at low frequencies \citep{2012arXiv1207.3025B}; {\bf provided that the
