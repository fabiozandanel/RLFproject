...
\abstract{The underlying physics of giant radio halos and \C{mini halos in galaxy clusters} 
is still an open question despite the growing number of detections. In this paper, we explore 
the possibility that radio-emitting electrons are generated in hadronic cosmic
ray (CR) proton interactions with ambient thermal protons of the intra-cluster
medium. Our CR model derives from cosmological cluster \C{hydrodynamical} simulations 
and additionally accounts for CR transport in the form of CR streaming and diffusion\C{.
This} opens the possibility of changing the radio halo luminosity by more than an
order of magnitude on a dynamical time scale. \C{We build a mock galaxy cluster catalog} 
from the large \C{MultiDark $N$-body $\Lambda$CMD simulation by adopting} a phenomenological
gas density model \C{for each cluster} based on X-ray measurements that matches Sunyaev-Zel'dovich
...
A2163) as well as radio mini-halos (Perseus, Ophiuchus)\C{,} while obeying upper
...
the Sunyaev-Zel'dovich flux, $Y$, \C{with respect to} gas density, our model is also 
able to simultaneously reproduce the observed bimodality of radio-loud and radio-quiet
...
\item The radio mini-halos in Perseus and Ophiuchus can be easily reproduced in
  ...
  shallower slope of the inner CR profile. To quantify the last point, we \C{try to
  ...
  $\gamma_{\rmn{tu}}=1$ still represents the best \C{match to the data}, demonstrating that
...
fractions between 10\% and 1\%. \C{In particular, we verified that the cumulative 
