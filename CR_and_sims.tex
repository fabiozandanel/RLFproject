...
\abstract{The underlying physics of giant radio halos and mini halos in galaxy
  clusters is still an open question despite the growing number of
  detections. In this paper, we explore the possibility that radio-emitting
  ...
  accounts for CR transport in the form of CR streaming and diffusion.  This
  opens the possibility of changing the radio halo luminosity by more than an
  order of magnitude on a dynamical time scale. We build a mock galaxy cluster
  catalog from the large MultiDark $N$-body $\Lambda$CDM simulation by adopting
  a phenomenological gas density model for each cluster based on X-ray
  measurements that matches Sunyaev-Zel'dovich and X-ray scaling relations and
  luminosity function. Our model successfully reproduces the observed surface
  brightness profiles of giant radio halos (Coma, A2163) as well as radio
  mini-halos (Perseus, Ophiuchus), while obeying upper limits on the gamma-ray
  emission in these clusters. Because of CR streaming transport and the
  different scalings of the X-ray luminosity, $L_{\rmn{X}}$, and the
  Sunyaev-Zel'dovich flux, $Y$, with respect to gas density, our model is
  also able to simultaneously reproduce the observed bimodality of radio-loud
  and radio-quiet clusters at the same $L_{\rmn{X}}$ as well as the unimodal
  distribution of radio-halo luminosity versus $Y$; thereby suggesting a
  physical solution to this apparent contradiction. For a plausible fraction of
  10\% radio-loud clusters, our model matches the NVSS radio-halo luminosity
  ...
  to probe the underlying physics of radio halos.}
...
In this appendix we describe in detail how our \emph{hybrid} model for the CR
distribution in galaxy clusters of Section~\ref{sec:2.3} is built from the
...
dominates, the CR distribution is modified. This can be shown analytically by
solving the continuity equation for CRs and obtaining the CR density profile,
$\rho_{\rmn{CR}}$, of Equation~(\ref{eg:rhoCR_1}). Assuming
$P(R)/P_{0}=n_{\rmn{e}}(R)/n_{0}$, i.e., neglecting the temperature dependence,
and adopting a standard $\beta$-profile for the electron density,
%
\begin{equation}
...
and $R < R_{-}$, respectively. \cite{2011A&A...527A..99E} obtain the CR
...
exist an analytical solution for Equation~(\ref{eg:rhoCR_1}) as in
...
exist an exact analytical solution for Equation~(\ref{eg:rhoCR_1}) as in
...
\caption{Comparison of the exact and approximate solutions for $C(R)$ in the
  ...
  \cite{2011A&A...527A..99E} but adopt
  $P(R)/P_{0}=n_{\rmn{e,GNFW}}(R)/n_{0}$. We show the GNFW profiles for NCCCs
  and CCCs, as derived in Section~\ref{sec:2.2}. For each of these profile
  class, we show three different cases of $\gamma_{\rmn{tu}}=100$, 10, and 1
  ...
  conservation during CR streaming. Note that the
  $C(10^{-3}R_{200},\gamma_{\rmn{tu}}=100)$ value in the CCC case is identical
...
Equation~(\ref{eq:jnu}), is (see also \citealp{2008MNRAS.385.1211P} and
\citealp{2011A&A...527A..99E})
%
\begin{equation}
...
proton-proton interactions $\sigma_{\rmn{pp}} =
...
\end{eqnarray}
%
where $x_{j}=\left[ 1 + \left( \frac{m_{\pi^{0}}c^2}{2E_{\gamma,j}}
  \right)^{2\sigma_{i}} \right]$, $\left[ \beta_{x}(a,b) \right]_{x_1}^{x_2} =
...
can affect this result, such as the small sample size that impact the resulting
