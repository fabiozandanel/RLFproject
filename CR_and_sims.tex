...
% Christoph Pfrommer\inst{2 \dagger} \and
% Francisco Prada\inst{3,4,1 \S}
%}
...
\begin{abstract}
%\abstract{
  The underlying physics of giant radio halos and mini halos in galaxy
  clusters is still an open question, which becomes more pressing with the
  growing number of detections. In this paper, we explore the
...
%\keywords{
\begin{keywords}
Cosmology: theory - Cosmology: diffuse radiation - Galaxies: clusters: intracluster medium 
- Radio continuum: galaxies - Gamma rays: galaxies: clusters - Astroparticle physics
\end{keywords}
%}
...
\citealp{1980ApJ...239L..93D,1982AJ.....87.1266V, 1999APh....12..169B,
  ...
  2003MNRAS.342.1009M,2003A&A...407L..73P, 2004A&A...413...17P, 2004MNRAS.352...76P,
...
boundary of some RHs. {\bf SENTENCE SHOULD BE SOFTENED: However, the recent report of spectral flattening with
...
observations in the GeV-band, see \citealt{2003ApJ...588..155R,
  2010JCAP...05..025A,2010ApJ...717L..71A,2012AAS...21920701Z,
  2012arXiv1207.6749H}; for ground-based observations in the energy band
...
without being able to detect cluster gamma-ray emission\footnote{{\bf Recently, \cite{2012arXiv1210.1574K} 
  claimed an evidence for diffuse gamma-ray emission in the VERITAS data of the Coma cluster \citep{2012...VERITAS}.
  However, this needs to be carefully scrutinized by the VERITAS collaboration itself.}}.  Gamma-ray limits
...
data on the Coma cluster \citep{2011arXiv1105.3240P,2012AAS...21920701Z,
  2012arXiv1207.6749H,2012...VERITAS}. To summarize, gamma-ray observations are not yet
...
the Universe. We adopt density parameters of $\Omega_{\rmn{m}}=0.3$,
...
{\bf We want to stress that our approach is not fully self-consistent and it is driven only 
by the ultimate goal of providing reliable gas densities and temperatures for all the
simulation halos.}
...
for gamma-ray observations \citep{2010MNRAS.409..449P,2011arXiv1105.3240P}. 
{\bf In Table~\ref{tab:RadioHalos}, we summarize the main characteristics of these halos
and detail the main parameters adopted for the modeling. In Figure~\ref{fig:SBmodeling}, we 
show the surface brightness and CR-to-thermal pressure profiles of each cluster together
with the allowed ($\gamma_{\rmn{tu}}$, $\alpha_{B}$) parameter space. All these clusters are
...
\hline
\phantom{\Big|}
 name & $z$ & $D$ & $L_{1.4~\rmn{GHz}}$ & references \\
\hline \\[-0.5em]
Coma           & $0.023$ & $101$ & $0.72$  &  [1, 2, 3]   \\
A~2163         & $0.203$ & $962$ & $15.36$  &  [3, 4]  \\
\hline \\[-0.5em]
Perseus        & $0.018$ & $78$   & $4.40$ &  [3, 5, 6]  \\
Ophiuchucs     & $0.028$ & $121$  & $0.19$  &  [3, 4] \\[0.5em]
\hline
\end{tabular}
\label{tab:RadioHalos}
\end{center}
\footnotesize{Note. {\bf Top two rows correspond to giant radio halos, while the
  bottom two rows are radio mini-halos. 
  %$\Delta d$ represents the approximate diameter of the RHs.
  $D$ is the luminosity distance in $h_{70}^{-1}$~Mpc. 
  $L_{1.4~\rmn{GHz}}$ is the observed total luminosity at 1.4~GHz in units of 
  $10^{31}$~$h_{70}^{-2}$~erg~s$^{-1}$~Hz$^{-1}$. 
  $M_{200}$ and $R_{200}$ are taken from \cite{2002ApJ...567..716R}. We
  ...
  brightness modeling. 
  %The model parameters column indicates the best match
  ...
  %corresponding predicted total luminosity at 1.4~GHz within $R_{200}$. 
  See the main text for details. References: [1] \cite{1997A&A...321...55D} [2]
...
\caption{Surface brightness modeling of the RHs in Coma, Abell~2163, Perseus and
  Ophiuchus. {\bf The left and central panels show the RHs' azimuthally averaged surface
  brightness profiles and the corresponding CR-to-thermal pressure profiles $X_{\rmn{CR}}(r)$,
  respectively, for representative model cases. In the central panel, we additionally mark
  the RHs' radial extension by a vertical bar. The rightmost panels show the reduced-$\chi^2$
  values, in the ($\gamma_{\rmn{tu}}$, $\alpha_{B}$) parameter space, of the fit (for Coma, it refers to 
  the 1.4~GHz case only). Naively, the smallest the value, the better the model can reproduce the data. 
  However, a reduced-$\chi^2$ value lower than 1 could mean an over-fitting of the data probably 
  because of the overestimation of the data point errors. The discontinuities that can be appreciated in the 
  rightmost panels at very small $\gamma_{\rmn{tu}}$ values are due to the approximate nature of the
  model. For Coma and Abell~2163, in the left panel, we additionally show the radial missing power of 
  the model with respect to the observational data.} 
  Note that different parameter values that yield almost the same surface brightness profile, result in 
  very different $X_{\rmn{CR}}$ profiles. {\bf We adopt a $10\%$ uncertainty range for the inner points 
  of the Coma $352$~MHz data.} In the case of Abell~2163 and Ophiuchus, we
  also adopt a $10\%$ uncertainty range instead of the errors reported by
  \protect\cite{2009A&A...499..679M} to account for additional systematic uncertainties,
  e.g., residual point source contamination. We also adopt a $10\%$ uncertainty
  range for Perseus, for which \protect\cite{1990MNRAS.246..477P} do not report any
  uncertainty intervals, {\bf and we only use the data points reported in \protect\cite{2004A&A...413...17P}.
...
match the radio observations. 
%The best match to the data is obtained for $\gamma_{\rmn{tu}}=1$ and $\alpha_{\rmn{B}}=0.6$.
{\bf From the study of the ($\gamma_{\rmn{tu}}$, $\alpha_{B}$) parameter space shown in 
Figure~\ref{fig:SBmodeling}, we can see than only very low values of $\gamma_{\rmn{tu}}$
provide a good match to the data, i.e., very flat CR profiles. A good match to the data is obtained 
for, e.g., $\gamma_{\rmn{tu}}=2$ and $\alpha_{\rmn{B}}=0.5$.} Values as high as $\gamma_{\rmn{tu}} 
\approx 4$ can be accommodated, however, at the expense of a shallower decline of the magnetic 
field profile (smaller $\alpha_B$) as a function of cluster-centric radius. {\bf With such values, we can 
recover the radio surface brightness shape and the total radio luminosity within about $25\%$.}

{\bf The gamma-ray flux (see Appendix~\ref{app:C}) within $R_{200}$ for
($\gamma_{\rmn{tu}} = 2$, $\alpha_{\rmn{B}}=0.5$) and for energies above 100~MeV (100~GeV) 
is $F_{\gamma} = 2.4 \times 10^{-9}$ ($8.7 \times 10^{-13}$) cm$^{-2}$~s$^{-1}$,
thus below the the gamma-ray limit recently set with the \emph{Fermi} data  
of $F_{\gamma, UL} (>100~\rmn{MeV}) \approx 3.2
\times 10^{-9}$~cm$^{-2}$~s$^{-1}$ \citep{2012...VERITAS}}.  
Note that for slightly higher values of $\gamma_{\rmn{tu}}$, i.e., a more centrally concentrated 
CR distribution, the radio and gamma-ray yield would be increased (assuming CR number
...
a level that is low enough to easily circumvent the gamma-ray constraints. {\bf In
fact, for, e.g., ($\gamma_{\rmn{tu}} = 3$, $\alpha_{\rmn{B}}=0.4$), we obtain 
$F_{\gamma} (>100~\rmn{MeV}) = 1.3 \times 10^{-9}$~cm$^{-2}$~s$^{-1}$ 
(and $F_{\gamma} (>100~\rmn{GeV}) = 4.9 \times 10^{-13}$~cm$^{-2}$~s$^{-1}$)}. 
We caution that in principle, CR streaming should cause the CR spectrum to steepen. 
...
that is suppressed in this picture in comparison to the lower-energy protons that the radio
emission is sensitive to \citep[see][for an extended discussion of this
point]{2011arXiv1111.5544M}.

{\bf However, such low values of $\gamma_{\rmn{tu}}$ are in discrepancy with our picture 
where clusters hosting giant radio halos should be characterized by high turbulence. 
Therefore, in Figure~\ref{fig:SBmodeling}, we additionally show the result for 
($\gamma_{\rmn{tu}} = 60$, $\alpha_{\rmn{B}}=0.5$) and the radial missing power 
of the model with respect to observations, while the total missing power is of about 35\%.

Recently, \citet{2012arXiv1207.3025B} point out problems of the hadronic model for 
the {\em limited} parameter space $\alpha\gtrsim2.6$, while adopting a much shallower 
azimuthally averaged radio halo profile \citep{2011MNRAS.412....2B} than the one used up 
to here  \citep{1997A&A...321...55D}. However, \citet{2004A&A...413...17P} demonstrate that 
$2.1\lesssim \alpha\lesssim2.5$ provide a fair description of the radio emission after taking 
into account the negative flux bowl due to the SZ effect that causes the measured radio halo 
spectrum to experience a sharp cutoff towards high frequencies.

We try to model also the  \cite{2011MNRAS.412....2B} $352$~MHz surface brightness 
profile corrected for residual point-source contamination (Rudnick, private communication). 
As shown with two examples in Figure~\ref{fig:SBmodeling}, our model cannot account for 
the total emission at $352$~MHz for any value in the ($\gamma_{\rmn{tu}}$, $\alpha_{\rmn{B}}$) 
parameter space. Interestingly, \citet{2012arXiv1207.3025B} need very flat CR profiles both for their 
hadronic and re-acceleration models in order to reproduce the Coma surface brightness 
at $352$~MHz, which corresponds to our finding.

The $\gamma_{\rmn{tu}}$ values that we would expect for giant radio halos cannot reproduce
the total emission of the Coma RH needing an increasing additional component with
increasing radial distance form the cluster center. This issue become more serious when
moving to lower frequencies, which, however, may be due to differences in the data reduction.  
We will come back to this crucial finding in Section~XX.
}

%Note that this analysis confirms the result by \citet{2012...VERITAS} who also
%conclude that the hadronic model for the Coma radio halo is an attractive
%explanation for magnetic field estimates inferred by Faraday rotation measure
%studies \citep{2010A&A...513A..30B} and is not challenged by {\em Fermi} upper
%limits on the gamma-ray emission if the full range of CR spectral indices is
%considered, i.e., for $2.1\lesssim \alpha\lesssim2.5$. \citet{2004A&A...413...17P}
%demonstrate that those spectral indices provide a fair description of
%the radio emission after taking into account the negative flux bowl due to the
%SZ effect that causes the measured radio halo spectrum to
%experience a sharp cutoff towards high frequencies.

%This is not necessarily in conflict with results by \citet{2012arXiv1207.3025B}
%who point out problems of the hadronic model for the {\em limited} parameter
%space $\alpha\gtrsim2.6$, while adopting a slightly shallower azimuthally
%averaged radio halo profile \citep{2011MNRAS.412....2B} than the one used in
%this analysis \citep{1997A&A...321...55D}. Careful work is needed to understand
%the origin of the observational discrepancy and to cleanly separate the radio
%halo emission from that of the Coma radio relic and other sources in the field.
%Interestingly, \citet{2012arXiv1207.3025B} need very flat CR profiles both for
%their hadronic and re-acceleration models in order to reproduce the Coma surface
%brightness which corresponds to our finding.
...
strength of $B_{0}=5~\mu$G. {\bf As in the Coma case, and even more extremely, 
only very low values of $\gamma_{\rmn{tu}}$ provide a good match to the data.
In Figure~\ref{fig:SBmodeling}, we show the case of} $\gamma_{\rmn{tu}}=1$ and 
$\alpha_B=0.3$, i.e., the flattest possible surface brightness. {\bf With this choice of 
parameters,} we recover the emission shape and the total luminosity within about $15\%$. 
The corresponding gamma-ray flux within 
$R_{200}$ is $F_{\gamma} (>100~\rmn{MeV}) = 4.2 \times10^{-10}$~cm$^{-2}$~s$^{-1}$, 
about two orders of magnitude lower than the upper limit obtained by \emph{Fermi}-LAT 
\citep{2010ApJ...717L..71A}, and 
$F_{\gamma} (>100~\rmn{GeV}) =1.5 \times10^{-13}$~cm$^{-2}$~s$^{-1}$.

{\bf As in the Coma case, such low values of $\gamma_{\rmn{tu}}$ are in discrepancy with 
our picture for giant radio halos. In Figure~\ref{fig:SBmodeling}, we show the result for 
($\gamma_{\rmn{tu}} = 60$, $\alpha_{\rmn{B}}=0.5$) and the radial missing power of the 
model with respect to observations. Here, the total missing power is much larger than in 
the 1.4~GHz Coma case, of about 80\%. We will come back to this in Section~XX.} 

\subsubsection{The Perseus radio mini-halo}

The diffuse radio emission in Perseus is the best known example of a radio
mini-halo \citep{1990MNRAS.246..477P}\footnote{{\bf We make use of 
   the \cite{1990MNRAS.246..477P} data instead of \cite{Sijbring1993}
   as the latter may be affected by residual point-source contamination.}} 
and Perseus itself is among the best studied clusters in X-rays
...
{\bf The ($\gamma_{\rmn{tu}}$, $\alpha_{\rmn{B}}$) parameter space study shows
that, as expected for mini-halos, low $\gamma_{\rmn{tu}}$ values are favored. 
However, a large portion of the parameter space, up to $\gamma_{\rmn{tu}} = 100$, 
can fit the data. Here it becomes clear the general anti-correlation between 
$\gamma_{\rmn{tu}}$ and $\alpha_{\rmn{B}}$: large $\gamma_{\rmn{tu}}$ values 
mean peaked CR profiles, low $\alpha_{\rmn{B}}$ values counteract this tendency 
in the final surface brightness profile.}

{\bf In Figure~\ref{fig:SBmodeling}, we show the two cases of ($\gamma_{\rmn{tu}}=3$,
$\alpha_{\rmn{B}}=0.4$) and ($\gamma_{\rmn{tu}}=100$, $\alpha_{\rmn{B}}=0.3$)}. 
We recover the surface brightness profile and the total luminosity within $10\%$.
...
$B_{0}=10$~$\mu$G. {\bf Similarly to the Perseus case, low 
$\gamma_{\rmn{tu}}$ values are favored, as expected for mini-halos. 
However, a large portion of the parameter space can fit the data.
In Figure~\ref{fig:SBmodeling}, we show the two cases of
($\gamma_{\rmn{tu}}=3$, $\alpha_B=0.4$) and 
($\gamma_{\rmn{tu}}=100$, $\alpha_B=0.3$).}
We recover the surface brightness profile and the total
luminosity within $20\%$. The gamma-ray flux within $R_{200}$ for the $\gamma_{\rmn{tu}}=3$
case and for energies above 100~MeV (100~GeV) is $F_{\gamma} = 1.3 \times
10^{-10}$ ($4.9 \times 10^{-14}$) cm$^{-2}$~s$^{-1}$. Adopting
...
{\bf THIS SHOULD BECOME THE NEW SECTION XX}
...
\item The magnetic field values adopted here are perfectly in agreement with
  ...
  \emph{classical} hadronic model (see, e.g.,
...
\item Considering the case of advection-dominated CR transport
  ...
  $B_{0,\rmn{Perseus}}\approx20$~$\mu$G and $B_{0,\rmn{Ophiuchus}}\approx1$~$\mu$G. We note,
...
rotation studies) {\bf as well as the theoretical considerations on CR transport
made in Section~XX}.
...
NCCCs and CCCs, respectively. {\bf THIS SHULD CHANGE DEPENDING 
ON WHAT WE SAY IN SECTION XX. We note that those values for $\gamma_{\rmn{tu}}$
would produce too concentrated surface brightness profiles for giant
...
{\bf It is important to stress that, for the radio-loud NCCCs, the additional contribution 
with respect to the hadronic one would decrease with decreasing masses, i.e., for less 
turbulent environments. This would mean a different, steeper, slope of the radio-loud 
NCCC population with respect to the others. We do not attempt to model this decrement 
due to the many uncertainties that this would imply, such as, e.g., different secondary 
contributions for different clusters as in the Coma and A2163 cases analyzed in 
Section~\ref{sec:3}. The aim of the next section is to provide predictions for the whole 
population of detectable RH in future radio surveys and, therefore, the impact of a 
steeper radio-loud NCCC population would not be the dominant factor.}
...
extended VLA, {\bf ASKAP \citep{2011PASA...28..215N} and APERTIF \citep{2012JApA..tmp...34R}} 
at 1.4 GHz and LOFAR at lower frequencies. Since the latter has already started to take 
data, it is extremely timely to present RLF predictions for this wavelength regime in our model, 
...
transport (or SZ decrement) nor a smaller hadronic contribution to the radio-loud
...
In particular, it should permit a robust determination of the number of clusters hosting 
RHs at a given luminosity (mass) and to carefully asses completeness issues. 
{\bf This will be extremely helpful in elucidating the RH generation mechanism, in 
determining the viable parameter space for the hadronic model and in establishing 
the precise role of the hadronic contribution to the total radio emission of merging clusters.}
...
streaming is necessary to solve the problems of the \emph{classical} hadronic scenario of
...
low-sensitivity radio instruments such as APERTIF, ASKAP, EVLA and SKA, 
in determining the RLF properties. In combination with future X-ray missions like 
\emph{e}ROSITA, this should yield a robust determination of the number of clusters hosting RHs at a
...
$Y_{\rmn{SZ}}$, $L_{120~\rmn{MHz}}$, $L_{1.4~\rmn{GHz}}$ and $L_{\gamma}$ (among
...
\section*{Acknowledgments}
  ...
  useful discussions. We also thank Gustavo Yepes, Jos\'e Alberto Rubi\~no,
  Frazer Pearce, Andrey Kravtsov, Harald Ebeling, Stefan Gottl{\"o}berg, Anatoly
  Klypin, Miguel-Angel Perez Torres, Adam Mantz {\bf and Huub R{\"o}ttgering} for the 
  useful discussions and advices. We thank Matteo Murgia for kindly providing the radio 
  surface brightness profiles of Ophiuchucs and Abell~2163 recomputed with respect to
  the Reiprich \& B\"{o}ringher (2002) cluster positions, and Wolfgang Reich for
  providing the {\bf 1.4~GHz} radio map of Coma. {\bf We thank Lawrence Rudnick and Shea 
  Brown for providing the 352~MHz radio surface brightness of Coma and for the useful
  discussions}. Finally, we thank the MultiDark database people, in particular Adrian Partl and 
  Kristin Riebe.  F.Z.{\ }acknowledges the CSIC financial support as a JAE-Predoc grant of the 
  program ``Junta para la Ampliaci\'on de Estudios'' co-financed by the FSE. F.Z.{\ }and F.P.{\ }
  thank the support of the Spanish MICINN's Consolider-Ingenio 2010 Programme under grant 
  MultiDark CSD2009-00064. C.P.{\ }gratefully acknowledges financial support of the 
  Klaus Tschira Foundation. The MultiDark Database used in this paper and the web 
  application providing online access to it were constructed as part of the activities of the 
  German Astrophysical Virtual Observatory as result of a collaboration between the 
  Leibniz-Institute for Astrophysics Potsdam (AIP) and the Spanish MultiDark Consolider 
  Project CSD2009-00064. The Bolshoi and MultiDark simulations were run on the 
