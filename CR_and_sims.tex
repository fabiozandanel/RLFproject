...
radio-to-X-ray and radio-to-SZ scaling relations. Hence, this explicitely
...
SZ-derived measurements.  Only if our model matches avialable cluster data on
the gas properties, we can start to explore different parametrizations of CR
physics and its implications for the radio and gamma-ray emission, explicited in
Section~\ref{sec:2.3}.
...
dynamical states at redshift $0.06<z<0.18$ with detailed informations of the
...
\propto r^3 P(r)$, peaks at scales slighly smaller than $R_{500}$ with
...
simulations have a weaker constraining power due to the comparably small boxsize
...
%The first assessed problem is how to assign a ICM gas density distribution, needed to calculate the radio and gamma-ray emission in the hadronic model, to the DM-only simulated MultiDark halos. 
...
%We chose those clusters as representative of the know RH population being Coma, Abell~2163 and Perseus very well known and Coma, Perseus and Ophiuchus the most promising targets for gamma-ray observations. 
...
%where 10\% of clusters are randomly defined as loud and 90\% as quiet (i.e.~with a radio luminosity significantly lower) assigning to them different values of $\gamma_{\rm{tu}}$, $\alpha_B$, the central magnetic filed $B_0$, and ensuring that the CR-to-thermal pressure ratio $X_{\rmn{CR}}$ is well below current observational constraints. We then 
 %
...
%For our model, we show that different fraction of loud-to-quiet clusters do not affect dramatically the RLF with the possible exception of the high luminosity (mass) range where however the very small statistic could affect such studies.  We also calculate the RLFs obtained applying the $L_{1.4~\rmn{GHz}}-L_{\rm{X,bol}}$ and $L_{1.4~\rmn{GHz}}-Y_{\rm{SZ}}$ observed relations to our mock catalog clusters using their $L_{\rm{X,bol}}$ and $Y_{\rm{SZ}, 500}$, as obtained from our phenomenological model, respectively. This is done using \emph{only} the halos defined as radio-loud clusters, not all of them, and it shows that in this case the lower luminosities (masses) are dramatically affected by different choices. 
...
%By fitting the RLFs at the different MultiDark snapshots with a functional form, we construct an analytical model including the redshift evolution for $n(L)$ for a representative realization of our CR model. %with a 10\% fraction of radio-loud clusters and 90\% fraction of radio-quiet (lower luminosity) clusters. 
%This permits to calculate the cumulative number of RH above a certain flux limit in all the Universe. 
