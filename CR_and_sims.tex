...
\begin{abstract}
  ...
  still an open question, which becomes more pressing with the growing number of
  detections. Assuming that the radio-emitting electrons are generated in
  hadronic cosmic ray (CR) interactions with protons of the intra-cluster medium
  explains the radio emission in mini halos (Perseus, Ophiuchus). By contrast,
  the hadronic model either fails to explain the extended emission of giant
  radio halos (Coma at low frequencies) or would require a flat CR profile 
  (Coma and A2163 at 1.4~GHz). Flat CR distributions are only expected to be realized 
  through CR streaming in {\em relaxed} clusters---in conflict with the observation that
  radio halos are hosted by merging {\em turbulent} clusters. This motivates us
  to propose a new hybrid model, where (i) mini halos are primarily of hadronic
  origin, (ii) giant halos experience a transition from central hadronic
  emission to peripheral leptonic emission due to Fermi I or II reacceleration
  of fossil or hadronically produced electrons, and (iii) steep spectrum radio
  sources are mainly of leptonic origin. We build a mock galaxy cluster catalog
  from the large MultiDark $N$-body $\Lambda$CDM simulation by adopting a
  phenomenological gas density model for each cluster based on X-ray
  measurements that matches Sunyaev-Zel'dovich and X-ray scaling relations and
  luminosity function. Because of CR streaming and the different scalings of the
  X-ray luminosity, $L_{\rmn{X}}$, and the Sunyaev-Zel'dovich flux, $Y$, with
  gas density, our model can simultaneously reproduce the observed bimodality of
  radio-loud and radio-quiet clusters at the same $L_{\rmn{X}}$ as well as the
  unimodal distribution of radio-halo luminosity versus $Y$; thereby suggesting
  a physical solution to this apparent contradiction. For a plausible fraction
  of 10\% radio-loud clusters, our model matches the NVSS radio-halo luminosity
...
challenges to explain the extend of giant radio halos within
...
Recently, \citet{2012arXiv1207.3025B} pointed out problems of the hadronic model
...
to the measured RH flux. This demonstrates that the magnitude of the SZ effect
is higher than assumed by \cite{2004JKAS...37..493B}, \cite{2004A&A...424..773R}, 
and \cite{2010MNRAS.401...47D}, while it is lower than the models of 
\cite{2002A&A...396L..17E} and \cite{2004A&A...413...17P}.

However, the much more extended RH profile at 352~MHz represents a serious
...
Fig.~\ref{fig:SB_Coma}) declines at a slightly faster rate towards the outskirts
than the profile used by \citet{2012arXiv1207.3025B} (shown in black). More
importantly, there is considerable azimuthal variation in the halo profile
\citep[see Fig.{\ }4 of][]{2011MNRAS.412....2B}, which would eventually have to
be modeled through hydrodynamical cluster simulations but which is beyond the
scope of this work.  Interestingly, as shown in Fig.~\ref{fig:SB_Coma}, the 1.4
GHz profile can be brought into agreement with the low-frequency profile by
choosing a lower zero level (Rudnick, priv. comm.; see also
\citealp{2012arXiv1208.3611P}), however at the expense of emerging super-cluster
emission from the Coma region on scales much more extended than the RH
\citep{2007ApJ...659..267K}. Conversely, by increasing the zero level of the 352
MHz data \citep{2011MNRAS.412....2B} reconciles that profile with the narrower
profile of \citet{1997A&A...321...55D} (Fig.~\ref{fig:SB_Coma}), possibly at the
expense of an implausibly high zero level, judging from the more sensitive 352
MHz data (Rudnick, priv.{\ }comm.).  Assuming for a moment that the choices of
zero level at both frequencies were indeed correct, this would imply a larger
extent of the RH at lower frequencies. A combination of better data at 1.4 GHz,
careful cleaning of the emission of tailed radio galaxies, and a full accounting
of the resulting systematic uncertainties into the total error budget are
certainly needed to fully clarify the observational situation.
...
\caption{Observed RH fluxes of the Coma cluster as compiled by
  ...
  discrepancy between RH flux measurements at different frequencies due to a
  lack of cross-frequency calibration.}
\label{fig:SZ_Coma}
\end{figure}
...
\caption{Surface brightness profiles of the Coma RH, normalized at 300 kpc.  We
  show a 352 MHz profile that was corrected for residual point-source
  contamination by applying the multi-resolution filtering technique as well as
  adopting the X-ray center for the RH profile (blue, Rudnick,
  priv. comm.). This profile declines at a slightly faster rate towards the
  outskirts than the profile used by \citet[][black
  squares]{2012arXiv1207.3025B}. Increasing the zero level of the (more
...
\citet{2011A&A...527A..99E}. These finding hint at the necessity of a second,
...
require a flat CR distribution both in Coma and in A2163 at 1.4~GHz---in 
contradiction to the expectation of peaked CR profiles in turbulent
...
\caption{Radio-to-X-ray and radio-to-SZ scaling relations in our extended CR model
  ...
  \emph{Right.} $L_{1.4~\rmn{GHz}}-Y_{\rmn{SZ}}$
...
space to match the observational constraints.  
...
Note finally that the detected signal of Mpc-scale diffuse emission in a stacked 
sample of radio-quiet galaxy clusters (green in left plot of Fig.~\ref{fig:PLSZ},
\citealp{2011ApJ...740L..28B}) agrees with the expected 
...
calibrating our model to 1.4~GHz observations may cause an over- or
...
comparison to the hadronic component in the radio-loud NCCC population,
respectively.
...
following new \emph{hybrid} hadronic-leptonic halo model.
\begin{enumerate}
